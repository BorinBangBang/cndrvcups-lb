...
void UpdateMediaBrandWithCurrMediaType(cngplpData *data);
	...
	if((ptr = ChkMainKey(buff, "*ModelName:", 10)) != NULL){
		...
		if(ppd_opt->printer_type == PRINTER_TYPE_FAX){
			if(ppd_opt->fax_setting == NULL){
				...
				cngplp_util_strcpy(ppd_opt->fax_setting->outside_line_number, "0");
			}
		}
...
void ResetUIDisable(UIItemsList *items_list)
{
	UIItemsList *tmp_list;
	...
	while(1){
		if(CurrDisable(data->ppd_opt->items_list, tmp->name) > 0){
			...
			if(strcmp(kPPD_Items_MediaType, tmp->name) == 0){
				UpdateMediaBrandWithCurrMediaType(data);
			}
		}
		if(tmp->next == NULL)
			break;
...
char* GetPPDDevOptionjobaccountbw(cngplpData *data, int id)
{
	char *glist = NULL;
	char tmp[256];
	switch(id){
